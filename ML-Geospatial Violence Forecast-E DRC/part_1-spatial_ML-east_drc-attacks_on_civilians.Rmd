---
title: 'ACLED Project: Eastern DRC Violence Prediction Replication Code'
author: "Rob Boswell"
date: "2025-01-17"
...
next_page <- ifelse(current_page < total_pages, paste0("part_", current_page + 1, "-spatial_ML-east_drc-attacks_on_civilians.html"), NA)
...
elevation_data <- get_elev_raster(locations = e_drc_adm2_map, z = 10, prj = st_crs(e_drc_adm2_map)$proj4string)


# Mask the elevation data to the shape of e_drc_adm2_map
elevation_data_clipped <- mask(elevation_data, as(e_drc_adm2_map, "Spatial"))
...
#### As of the latest reports, there are approximately [120 armed groups active in the Eastern DRC's Ituri, North Kivu, South Kivu, and Tanganyika provinces.](https://www.hrw.org/world-report/2023/country-chapters/democratic-republic-congo). These groups include fighters from neighboring countries such as Rwanda, Uganda, and Burundi.[Between January and late October 2023, various armed actors killed at least 2,446 civilians](https://www.hrw.org/world-report/2023/country-chapters/democratic-republic-congo) in South Kivu, North Kivu, and Ituri provinces.
...
#### The complex dynamics of the Eastern DRC conflict naturally make forecasting violence against civilians a difficult task. This portfolio project takes on this challenge, not using time series, though I will do so in a future project. While some analysts would simply refer to prior locations of violence on a Kernel Density Estimation (KDE) heat map as indicative of where violence will likely occur in the future, I take a geospatial machine learning (ML) approach: predicting violence even in areas where violence may not have occurred yet.

<br>

#### Using geospatial ML models are often better than regular ML models at predicting target variables. First, the former explicitly account for the relationships and dependencies between neighboring grid cells. By recognizing that nearby locations influence each other, they can capture local patterns and trends more effectively. In contrast, traditional ML models treat each data point independently, ignoring the spatial relationships that are crucial for accurate predictions. Second, geospatial models often include spatial features such as distance to landmarks, elevation gradients, or spatial lag variables that enhance the model's understanding of the geographical context. Third, by modeling spatial autocorrelation, these models can reduce spatial bias and variance, leading to more reliable and robust predictions at the local level.

<br>

#### Specifically, I focus on 3 ML regression algorithms - Extreme Gradient Boosting (XGBoost), Random Forest (RF), and Support Vector Machines (SVM's) to forecast locations and numbers of attacks on civilians within grid cells, each comprising 10 square miles. Unlike some of my other projects, this project will not use time series models, as it is meant for demonstration purposes. However, any true forecast must include temporal data.

<br>

#### Using KDE as the baseline, at the end of the project, I will compare its predicted spatial risk categories with those from geospatial ML models by examining the proportion of actual civilian attacks in each category, in order to determine whether KDE or geospatial models capture more attacks in their predictions.

<br>

#### Since selecting the optimal cross-validation strategy for geospatial data is less explored than choosing effective algorithms, I will start by using XGBoost as a baseline and compare nine different geospatial cross-validation strategies. To conserve computational resources, I will then apply the best-performing cross-validation strategy with two additional algorithms - random forest (RF) and support vector machines (SVMs) - to assess further performance improvements. Focusing on minimizing the absolute differences between predicted and actual attack numbers, I will rank each model accordingly. Additionally, recognizing that model performance varies across different map regions, I will combine the best-performing model for each second-level administrative district and conduct further analyses on these final predictions.

<br>

#### Note: A critical goal is to recognize that predicting the number of attacks on civilians by armed actors - especially in grid cells with an area of only 10 square mi. - is challenging since the data is highly skewed towards 0 attacks. Thus, I will use dynamically weighted metrics for hyperparameter tuning, and multi-objective tuning to enhance predictive performance. This project does not compare regular ML model performance to geospatial model performance. Future studies may do so, but potential biases inherent with not accounting for spatial autocorrelation, with regular ML, should be considered.

---

### **Dynamically Weighted Metrics:**
...
#### Errors in grid cells with more important or rare target values (values greater than 0) are given more weight, so the model is penalized more heavily for mistakes in these areas.

<br>

#### Errors in grid cells with common or less important target values (a value of 0) are given less weight, so the model is not overly focused on minimizing these errors at the expense of others.

<br>

#### This dynamic weighting ensures that the model focuses on reducing errors in the most critical parts of the dataset.
...
#### I also will be using multi-objective tuning rather than standard single objective tuning. Using multi-objective tuning with dynamically weighted metrics offers several advantages, especially in the context of highly skewed geospatial data. 

<br>

#### **1) Balancing Different Types of Errors:**

#### With single-objective tuning, the model may focus excessively on optimizing a specific type of error, such as minimizing overall error (e.g., using RMSE) or absolute differences (e.g., using MAE), at the expense of other important error characteristics.

<br>

#### Multi-objective tuning, on the other hand, allows the model to strike a balance between different types of errors. E.g., dynamically weighted RMSE captures large errors by emphasizing significant mispredictions in grid cells where rare, high target values occur, while dynamically weighted MAE focuses on the average magnitude of errors across all grid cells but gives additional weight to the few areas with high target values. This ensures that both large and typical errors are prioritized.

<br>

#### **2) Mitigating the Impact of Imbalanced Data:**

#### With highly imbalanced data, focusing solely on one metric (like dynamically weighted RMSE) could cause the model to overly prioritize reducing large errors in cells with high values, potentially at the expense of capturing smaller but more frequent errors in the lower value cells.

<br>

#### On the other hand, tuning using a metric like dynamically weighted MASE can help the model improve accuracy in the few grid cells with higher counts by adjusting for varying scales of error, while simultaneously tuning using dynamically weighted Huber loss enhances robustness by remaining sensitive to both smaller errors in the many zero-count cells and larger outliers in cells with higher counts.

<br>

#### **3) Improving Spatial Generalization:**

#### My spatial data likely exhibits spatial autocorrelation, meaning nearby locations are likely to have similar values. Multi-objective tuning can ensure the model generalizes well across various spatial regions, not overfitting to one region or spatial pattern.
...
#####  - *Near-Optimal Solutions:* Since the Pareto front typically contains only a few solutions at most, using PCA weights solely on this small set - later to be used for ranking which models are best - might not be very representative. Therefore, the code considers a specified number (50) "near-optimal" solutions that include those identified as either close to the Pareto front or on it. The Euclidean distance of each solution to the Pareto front is calculated for this purpose. Including these solutions helps make the PCA weights more representative of the entire solution space, providing a better understanding of the importance of each metric across a broad set of potential hyperparameter settings.
...
####  PCA is used to A) determine the level of importance of each metric in the set of Pareto optimal and near-Pareto optimal solutions, and B) identify which among the multiple solutions likely existing on the Pareto front contains the best hyperparameter set:
...
##### 2) Combining Principal Components: The code uses the first two principal components (PC1 and PC2) because they capture almost all the variance in my case. The loadings (weights) of these components are combined to calculate the importance of each metric.
...
##### 4) Selection of Best Hyperparameters: The hyperparameter set of the Pareto front solution with the lowest composite score is considered the best.
...
#### *Note:* While normally it is better to compare model performance in terms of test set metrics rather than training set tuning metrics to more accurately replicate out-of-sample performance, due to the use of multi-objective tuning in this project, and the fact that I use PCA to help identify the order of importance of each metric for any given model, we are able to compare performance based on tuning metrics using composite scores much more easily than comparing the performance based on test set metrics, as PCA is not applied to the test set metrics. It would be difficult to know whether all of the 34 test set metrics I will use are equally important across all models, or whether some metrics are more important for some models than others, thus providing a potentially unfair comparison.

<br>

#### Some of the test set metrics are standard, while others are dynamically weighted, and some are both asymmetric and dynamically weighted, penalizing unperpredictions more than overpredictions. In the context of attacks on civilians, it would be natural to assume the costs of underpredicting violence is greater than of overpredicting it. Asymmetric metrics elevate the importance of reducing false negatives (underpredictions) in the overall performance assessment. This means that models with fewer underpredictions will score better, even if they have more overpredictions. This can help us understand which models may need improvement. Further, if tuning solely with dynamically weighted metrics results in too many underpredictions or overpredictions, in future projects I may experiment with tuning with custom metrics which are both dynamically weighted and asymmetric (penalizing either underpredictions or overpredictions, as needed).
...
#### - **Mean population density**, from the Gridded Population of the World (GPW), v4 - UN WPP-Adjusted Population Density, v4.11 (2020) dataset, by the Socioeconomic Data and Applications Center (SEDAC).
