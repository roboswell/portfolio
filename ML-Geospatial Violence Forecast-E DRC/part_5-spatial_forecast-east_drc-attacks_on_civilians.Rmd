...
```{r ROC Curve and AUC Score, cache=TRUE, cache.lazy=FALSE, fig.height=10.5, fig.width=10.5, eval=FALSE, echo=TRUE}
...
```{r ROC Curve and AUC Score - Plot, cache=TRUE, cache.lazy=FALSE, fig.height=10.5, fig.width=10.5, eval=TRUE, echo=FALSE, out.width="100%", fig.align='center'}
...
#### The AUPRC score represents a summary of the PR curve, measuring the average trade-off between Precision and Recall across all thresholds. In an ideal scenario, AUPRC = 1 would indicate perfect precision and recall. A random classifier would achieve an AUPRC value close to the prevalence of the positive class, which in our case is the proportion of grid cells with attacks (around 0.017, according to my calculations in the code chunk below).

<br>

#### Given that a random classifier would have an AUPRC of approximately 0.017, the fact that our model achieves an AUPRC of **0.4835** is a **substantial improvement** over the baseline. While this AUPRC might seem low in absolute terms, it is actually **strong performance** relative to the prevalence of the positive class. Here is why:

<br>

#### - AUPRC of 0.4835 is much higher than 0.017 (the baseline for a random classifier). This indicates that the model is capable of significantly better-than-random predictions for attack locations, despite the class imbalance. However, the model's performance is still far from perfect (AUPRC = 1), reflecting the ongoing challenge of improving precision and recall given the imbalance.
...
```{r Precision-Recall Curve and Area Under the Precision-Recall Curve Scores, cache=TRUE, cache.lazy=FALSE, fig.height=10.5, fig.width=10.5, eval=FALSE, echo=TRUE}

# First, let's calculate the percentage of attack grid cells among all grid cells in our map:

# Calculate the total number of rows in data_combined
total_rows <- nrow(data_combined)

# Calculate the number of rows where countAttacks is equal to 1 or higher
count_greater_than_or_equal_1 <- sum(data_combined$countAttacks >= 1)

# Calculate the ratio
attck_cell_ratio <- count_greater_than_or_equal_1 / total_rows

# Print the result
print(attck_cell_ratio)
# > print(attck_cell_ratio)
# [1] 0.01617315
...
plot(pr, main = "Precision-Recall Curve\n(Color Gradient: Thresholds at which Precision & Recall are Computed)")

```

<br>

```{r Precision-Recall Curve and Area Under the Precision-Recall Curve Scores - Plot, cache=TRUE, cache.lazy=FALSE, fig.height=10.5, fig.width=10.5, eval=TRUE, echo=FALSE, out.width="100%", fig.align='center'}
...
#### However, most of the other territories have **models with mixed or mostly poor classification metric results**, such as **Rutshuru** (0.669 ROC AUC, 0.328 AUPRC, 0.353 F1-Beta, and 0.353 F2-Beta), **Fizi** (0.802 ROC AUC, 0.315 AUPRC, 0.455 F1-Beta, and 0.455 F2-Beta), and **Masisi** (0.647 ROC AUC, 0.311 AUPRC, 0.345 F1-Beta, and 0.446 F2-Beta), **among others**.
