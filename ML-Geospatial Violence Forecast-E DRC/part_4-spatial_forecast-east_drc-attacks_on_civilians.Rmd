...
#### Before building my geospatial models, I will create custom metrics - those not built into mlr3 - for hyperparameter tuning and model evaluation on test data. Recall that the need for creating dynamically weighted metrics results from the high level of skewness of our target variable.
...
#### The quantile distribution shows that up to the 80th percentile, the target variable is 0. At the 90th percentile, it increases to a value of 1. This means that 90% of the data points have values ≤ 1. Between the 90th and 100th percentiles of data points, the target variable sharply increases from 1 to 183. Earlier in this project, we found that 7,366 (or 92%) of the training set grid cells contain 0 attacks on civilians, and that 8,273 (or 98%) of the test set grid cells also contain zero attacks.

<br>

#### By addressing skewness using dynamically weighted custom metrics, we enable our models to better handle the data's inherent imbalances, leading to more accurate and reliable geospatial predictions. 

<br>

#### Note that while it is common practice to use resampling strategies (e.g., oversampling, undersampling, or SMOTE) to balance many forms of highly skewed data, this is not the case with geospatial data. Because resampling strategies can disrupt the spatial dependencies and autocorrelation inherent in highly skewed geospatial data, potentially introducing bias and invalidating statistical assumptions models rely on to make accurate predictions, most data scientists avoid using them.
...
# The tau parameter in the Quantile Loss function represents the quantile to be estimated. It is a value between 0 and 1. When tau is set to 0.5, it corresponds to the median, and the Quantile Loss function is thus similar to the Mean Absolute Error (MAE).
...
#### Soon, I will create our first geospatial ML model. Choosing the right cross-validation technique when working with geospatial data is very important. The first technique we will explore is leave-one-group-out (LOGO) spatial cross-validation, in which each 2nd order aministrative district (i.e., East DRC territory) will one-by-one be held out as a test fold, while all others together will act as the training set, to make predictions. In mlr3, this is implemented using the `custom_cv` resampling function. This map represents each territory as a separate fold for `custom_cv` LOGO cross-validation.
...
ggplot() +
  geom_sf(data = final_net.sf.attacks_train, aes(fill = NAME_2), linewidth = 0.3, show.legend=FALSE) +
  scale_fill_viridis(option = "magma", discrete = T,
                    name="Attacks Count") +
  geom_sf(data = e_drc_adm1_map, fill = NA, color = "red", linewidth = 1) +
...
districts <- unique(attacks_train_Spatial$NAME_2)
...
for (d in districts) {
  ...
  district_indexes <- which(attacks_train_Spatial$NAME_2 == d)
...
### **Code for Geospatial ML Models:**

<br>

#### The following code for `custom_cv` Leave-One-Group-Out (LOGO) cross-validation with XGBoost regression uses the NAME_2 column, which contains the names of the second order administrative district (territory) associated with each grid cell.
...
# Create a first order administrative map from the above second order administrative map
...
data.regr.test <- data.regr.test %>% dplyr::select(-countAttacks, -NAME_2)
...
task_regr_train$set_col_roles("NAME_2", "group")
...
resampling_regr$instantiate(task_regr_train, col = "NAME_2")
...
data.regr.train <- data.regr.train %>% dplyr::select(-countAttacks, -NAME_2)
data.regr.test <- data.regr.test %>% dplyr::select(-countAttacks, -NAME_2)
...
data.regr.train.nested <- data.regr.train %>% dplyr::select(-countAttacks, -NAME_2)
data.regr.test <- data.regr.test %>% dplyr::select(-countAttacks, -NAME_2)
...
features_to_keep.combined <- union(c("uniqueID", "NAME_2", "countAttacks"), features_to_keep.combined)
...
features_to_keep.gain <- union(c("uniqueID", "NAME_2", "countAttacks"), features_to_keep.gain)
...
features_to_keep.perm_imp <- union(c("uniqueID", "NAME_2", "countAttacks"), features_to_keep.perm_imp)
...
combined_attacks <- attacks_by_territory_actual %>%
  ...
  left_join(attacks_by_territory_predicted %>% 
              ...
              rename(Predicted = total_attacks), by = "NAME_2") %>%
  tidyr::gather(key = "Type", value = "Total_Attacks", Actual, Predicted)

# Rearrange the order of bars
combined_attacks$NAME_2 <- factor(combined_attacks$NAME_2, levels = c("Beni", "Irumu", "Djugu", "Rutshuru", "Mambasa", "Goma", "Uvira", "Fizi", "Masisi", "Kalehe"))

# Data for the vertical separators
separator_positions <- data.frame(
  x = seq(1.5, length(levels(combined_attacks$NAME_2)) - 0.5, by = 1)
)
...
ggplot(combined_attacks, aes(x = NAME_2, y = Total_Attacks, fill = Type)) +
...
# 
# # Create the bar chart with modifications
# g_plot_12 <- ggplot(combined_attacks, aes(x = NAME_2, y = Total_Attacks, fill = Type)) +
#   geom_bar(stat = "identity", position = "dodge") +
#   geom_text(aes(label = round(Total_Attacks)), 
#             vjust = 1.5,
#             color = "black", 
...
# 
# # Create the bar chart with modifications
# g_plot_13 <- ggplot(combined_attacks, aes(x = NAME_2, y = Total_Attacks, fill = Type)) +
#   geom_bar(stat = "identity", position = "dodge") +
#   geom_text(aes(label = round(Total_Attacks)), 
#             vjust = 1.5, 
#             color = "black", 
...
# 
# # Create the bar chart with modifications
# g_plot_15 <- ggplot(combined_attacks, aes(x = NAME_2, y = Total_Attacks, fill = Type)) +
#   geom_bar(stat = "identity", position = "dodge") +
#   geom_text(aes(label = round(Total_Attacks)), 
#             vjust = 1.5, 
#             color = "black", 
...
#### Context-Specific Modeling: This approach aligns with the concept of context-specific modeling, where different models are applied to different subsets of data based on their unique characteristics. This is a well-accepted practice in ML, especially in complex, heterogeneous environments like the Eastern DRC. This strategy is similar to that used in land change modeling, where different models are often used for different geographical areas depending on the characteristics of the region and the specific goals of the study. E.g., when studying land change in different regions, modelers may choose to use agent-based models, ML models, or cellular automata models depending on which model best captures the unique patterns and processes of that particular region.
...
# Loop through each unique territory in NAME_2
for (territory in unique(final_net.sf.attacks_test$NAME_2)) {
  ...
  for (file in prediction_files) {
    ...
    filter(uniqueID %in% final_net.sf.attacks_test$uniqueID[final_net.sf.attacks_test$NAME_2 == territory])
...
# }
# 
# g_plot_16 <- ggplot(combined_attacks, aes(x = NAME_2, y = Total_Attacks, fill = Type)) +
#   geom_bar(stat = "identity", position = "dodge") +
#   geom_text(aes(label = custom_round(Total_Attacks)),
#             vjust = 1.5,
#             color = "black",
...
#### Only 0.7% of actual attacks occurred in the areas KDE identified as having the lowest risk (1% to 29% risk) compared to 8.6% actual attacks for areas the combined ML models predicted to contain the lowest risk. This is the only risk category in which KDE performed better than the combined models. A smaller percentage of actual attacks occurring in low-risk zones indicates better performance in correctly predicting safe areas.
...
# Step 2: Prepare the ML predictions data frame
...
x_nudge1 <- max(e_drc_adm2_map$COORDS_X) + 0.2 * (max(e_drc_adm2_map$COORDS_X) - min(e_drc_adm2_map$COORDS_X))
x_nudge2 <- max(e_drc_adm2_map$COORDS_X) - 0.2 * (max(e_drc_adm2_map$COORDS_X) - min(e_drc_adm2_map$COORDS_X))
...
label_positions <- e_drc_adm2_map %>%
  filter(NAME_2 %in% c(group1, group2)) %>%
  mutate(
...
```{r Maps-Actual vs. Predicted Attacks-Combined Optimal Models for Each Territory - Plot, cache=TRUE, cache.lazy=FALSE, fig.height=10.5, fig.width=10.5, eval=FALSE, echo=FALSE, out.width="100%", fig.align='center'}
...
```{r Maps-Actual vs. Predicted Attacks-Combined Optimal Models for Each Territory - Plot PNG, cache=TRUE, cache.lazy=FALSE, fig.height=10.5, fig.width=10.5, eval=TRUE, echo=FALSE}

# DELETE in Final Version

knitr::include_graphics("final_plot-map_of_actual_vs_predicted_attacks.png")
