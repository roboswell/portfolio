...
previous_page <- ifelse(current_page > 1, paste0("part_", current_page - 1, "-spatial_ML-east_drc-attacks_on_civilians.html"), NA)
...
e_drc_adm2_map <- st_transform(e_drc_adm2_map, crs(merged_raster))
...
shapefile_extent_sp <- as(extent(st_bbox(e_drc_adm2_map)), "SpatialPolygons")
crs(shapefile_extent_sp) <- crs(e_drc_adm2_map)
...
e_drc_adm2_map_sp <- as(e_drc_adm2_map, "Spatial")

# Crop the raster to the extent of the shapefile
cropped_raster <- crop(merged_raster, e_drc_adm2_map_sp)

# Masking the cropped raster using the shapefile
clipped_raster <- mask(cropped_raster, e_drc_adm2_map_sp)
...
# After masking, the number of NAs jumps from 15,019 to ~1.37 million. Why the jump? This is because crop() and mask() set any pixel that is outside the shapefile boundary to NA.
...
crs(clipped_raster) <- crs(e_drc_adm2_map)
...
# Note that projectRaster() can introduce additional interpolation artifacts or small fill areas where data might become NA, which may explain the additional increase in NAs.

# 1396027/2283463 = 0.611364. 61.14% of all values are NAs. This is a lot of NA values, a potential limitation of using night light emissions as a feature. However, my comments on this subject further below show that it may still be okay to use this variable.
...
# We might suspect that many of these NAs are present because the original raster was clipped to the area of the map shapefile, leaving most values outside the shapefile as NAs. However, because there are so many NA values, to verify whether these NA values are merely coming from areas outside the shapefile, we should do the following:
...
# 1. Create mask_raster where any cell inside the shapefile boundary = 1, else NA.
# 2. Check how many non-NA cells in merged_raster become NA in mask_raster.
# 3. If outside_values_count = 0, then none of the original valid raster cells lie outside the shapefile boundary. They all lie within it.

outside_values_count = 0 => none of the original valid raster cells lie outside the shapefile boundary.


# Here is the R code to perform this analysis:

# Create a mask raster from the shapefile
mask_raster <- rasterize(e_drc_adm2_map, merged_raster, field=1, background=NA)
...
# From print(summary(final_values)), we saw there are 1,396,027 NAs. From print(outside_values_count), we can see there are 0 NAs. This means that 0% of the NAs in final_values come from areas outside the shapefile boundaries.
...
# How is this possible that we only have 3 NA values now? 

# - Because a single polygon in the fishnet typically covers many raster cells, the presence of a large number of per-pixel NA values does not necessarily translate into a large number of fishnet polygons with missing values.

# - Many missing pixels may still yield a valid average. Each fishnet cell (polygon) might contain hundreds or thousands of raster pixels. Even if some of those pixels are NA, there may be enough valid (non-NA) pixels within that polygon to compute a mean.

# - You only see an NA in the final fishnet attribute table if every pixel (or almost every pixel) in a given fishnet polygon is missing. Hence, you can have 1.3 million missing pixels scattered across the region but, if they are “sprinkled” throughout different polygons (which also contain some valid pixel values), most if not all polygons can still compute a mean.

# Next, for imputation purposes for NA values, the following code defines which cells are neighbors by creating a spatial weights matrix (Queen's contiguity matrix). The poly2nb function identifies neighboring polygons (grid cells), and nb2listw creates the weights. The style="W" argument creates weights based on neighboring presence (1 for neighbor, 0 otherwise), and zero.policy=TRUE allows operations on regions with no neighbors.
...
print(sum(is.na(mean_light_net_merged$mean_light))) # Results show there are now 0 NAs.
...
ggplot(data = vars_in_net) +
  ...
  geom_sf(data = e_drc_adm2_map, fill = NA, color = "white", size = 0.1) + # Territory Boundaries (2nd Order Districts)
  geom_sf(data = e_drc_adm1_map, fill = NA, color = "red", linewidth = 1) + # Province Boundaries (1st Order Districts)
...
# I merged the raster files in QGIS (since doing so takes much longer in R) that provided travel times to cities of population sizes 50,000 - 100,000; 100,000 - 200,000; 200,000 - 500,000; and 500,000 - 1,000,000, based on 2015 data. I then clipped the merged raster file to the boundaries of e_drc_adm2_map (which has the same boundaries of fishnet). The result is the imported raster file below. This raster file thus contains travel times from any given point on the map to any city with a population between 50,000 and 1,000,000.
...
# I merged these raster files together and then clipped them to the boundaries of e_drc_adm2_map using QGIS, since QGIS has a processing time much faster than R's. The merged/clipped raster is imported below:
...
# Create spatial weights using IDW. This weights matrix can be reused for other spatial objects with centroids regardless of differences in values, because the weights matrix depends on the locations of the centroids - not their values. This will save time.
...
# There is no need to create a new neighbors_weights matrix object. The previous neighbors_weights can simply be reused in the weighted_harmonic_avg_dists() function below for the reasons previously explained.
...
# The previous neighbors_weights can simply be reused in the weighted_harmonic_avg_dists() function below for the reasons previously mentioned.
...
# If we simply substituted 0 instead of Inf, this would incorrectly imply the average distance is zero, which has a different meaning (suggesting that the NSAGs are exactly at the location of the cell in question, which would not be the case here).
...
# In theory, we could also use just nsags_count_sf_centroids_train to place in both train and test set versions of the weighted_harmonic_avg_dists() function below since nsags_count_sf_centroids_train and nsags_count_sf_centroids_test share exactly the same spatial coordinates. However, for clarity, I use nsags_count_sf_centroids_train in the training set version of the weighted_harmonic_avg_dists() function and nsags_count_sf_centroids_test in the test set version of weighted_harmonic_avg_dists().
...
total_land_seizures_intersection_no_geom_train <- as.data.frame(total_land_seizures_intersection_train) %>% 
  dplyr::select(-c(geometry)) # Delete geometry in order to perform the left join below
...
# Count the number of NA values to make sure there are not many.
...
unhcr_e_drc <- st_intersection(unhcr_drc, e_drc_adm2_map)
...
vars_in_net_train <- vars_in_net_train %>% dplyr::select(-TYPE_3, -ENGTYPE_3, -NL_NAME_2, -VARNAME_2)
vars_in_net_test <- vars_in_net_test %>% dplyr::select(-TYPE_3, -ENGTYPE_3, -NL_NAME_2, -VARNAME_2) 
...
#### - *Make the NAME_2 column consisting of 2nd order administrative districts (territories) a factor*.

<br>

```{r Convert NAME_2 Column to Factor, cache=TRUE, cache.lazy=FALSE, fig.height=10.5, fig.width=10.5, eval=FALSE, echo=TRUE}

final_net.sf.attacks_train <- final_net.attacks_train %>%
  mutate(NAME_2 = factor(NAME_2))
...
#### Before conducting feature engineering involving Local Moran's I calculations, let's begin with an exploratory analysis. We will create the following four maps and display them together in a faceted layout:
...
#### While Global Moran's I values are always between +1 and -1, this is not true of Local Moran's I values. If I were to not use IDW weights for my Queen's contiguity matrix, the Local Moran's I values would be much higher. IDW is a commonly used method to reflect the idea that spatial influence decreases as distance increases. This is a reasonable assumption in this project, as attacks occurring in one region are more likely to affect nearby regions more than distant ones. 

<br>

#### The alpha parameter controls the rate of decay. In my case, alpha = 2 means that spatial influence diminishes quadratically with distance. A higher alpha (e.g., alpha = 3) might overly penalize distant neighbors, potentially ignoring relevant distant spatial interactions. Conversely, a lower alpha (e.g., alpha = 1) might not sufficiently reduce the influence of local points, leading to an overestimation of spatial dependence. Alpha = 2 strikes a balance.

<br>

#### Local Moran's I values are more likely to be higher when using the row-standardized contiguity-based weighting matrix (style = "W") compared to IDW with alpha = 2. First, this is because non-IDW contiguity-based weights tend to capture broader local spatial patterns by giving each neighbor relatively equal importance. However, it may not be realistic for events transpiring hundreds of miles away to be as likely to have the same impact on the number of attacks occurring against civilians as events transpiring only a few miles away.

<br>

#### Second, this type of weights matrix ensures the sum of weights for each observation equals 1. The calculation of Local Moran's I involves multiplying the deviation of each observation by the weighted sum of the deviations of its neighbors. If certain observations have values that are significantly higher than the mean and are surrounded by neighbors with similarly high values, the Local Moran's I for that point can be quite large.

<br>

#### Third, IDW with alpha = 2 strongly diminishes the influence of distant neighbors, focusing mostly on close neighbors. This can result in very low Local Moran's I values because distant points are assigned very little weight, and the number of influential neighbors is small. 

<br>

#### **3rd Map (2nd Row, 1st Column) - P-Values:** This map shows the p-values associated with the Local Moran's I statistic. Areas with low p-values (dark blue areas) suggest that the observed spatial pattern (whether a hot spot or cold spot) is statistically significant, whereas areas with high p-values (green and yellow areas) suggest the observed pattern could be due to random spatial processes. We can see most areas on the map are not statistically significant, with the exception of the blue and purple clusters mostly along the eastern parts of the map.
...
#### Now, having performed exploratory data analysis with Local Moran's I values, I will add 2 more features: 
...
ggplot() +
  ...
  geom_sf(data = e_drc_adm1_map, fill = NA, color = "grey", linewidth = 1) +
