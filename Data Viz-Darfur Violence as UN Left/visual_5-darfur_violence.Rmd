...
acled_2022 = subset(acled_short, year == 2022)
...
geo_pko_for_maps_22 = filter(geo_pko, year == 2022)


geo_pko_df_22 = geo_pko_for_maps_22[c("year","location","latitude", "longitude")]

geo_pko_22_avg_troops_per_base = geo_pko_for_maps_22 %>% group_by(location) %>% summarise(avg.troops = mean(no.troops, na.rm=T))

geo_pko_df_22 = inner_join(geo_pko_22_avg_troops_per_base, geo_pko_df_22, by="location")
geo_pko_df_22 = distinct(geo_pko_df_22)

active_bases_2022 = geo_pko_df_22



#Note: In my 2023 portfolio version of the following UN closed bases files, I do not include sites that were closed in July or after of the given year since July is the 7th month of the year - meaning such bases were active for at least half of the given year.
...
closed_bases_2022 <- read_csv("closed_bases_2022-2023_portfolio.csv")
...
annotate("text", x= 24.420469, y= 11.002820, label="South Darfur", fontface=2,size=3) + 
annotate("text", x= 23.116657, y= 14.519303, label="West Darfur", fontface=2,size=3) + 
stat_density2d(data=acled_2022, geom = "polygon", aes(x=longitude, y=latitude,fill=..level.., alpha=..level..)) + 
guides(alpha="none") + 
geom_point(data = active_bases_2022, mapping = aes(x = longitude, y = latitude, size=1.5, alpha=0.5), shape=22, color="steelblue") +
guides(size="none")  + 
geom_point(data = closed_bases_2022, mapping = aes(x = longitude, y = latitude, size=1.5, alpha=0.3), shape=25, color="darkgreen") +
guides(size="none")  +
theme(legend.position="none", title =element_text(size=13, color=  "steelblue",face='bold'), axis.title.x=element_blank(),              axis.title.y=element_blank(), axis.text.x = element_blank(), axis.text.y = element_blank(), rect = element_blank()) + 
labs(title="2020", subtitle="", caption = "") + 
guides(fill=guide_legend(title="Concentration of All\nAttacks on Civilians"))
...
#Note: the code for adding a title and text at the bottom of the visual was aided by code found on: https://stackoverflow.com/questions/50783436/ggplot2-multiple-plots-with-shared-legend-one-background-colour-1-main-and-3
