...
#The first step will be to take the active_bases shapefiles I exported earlier, and to spatially join them to a West Darfur polygon which contains second level administrative regions within it. I thus create a West Darfur spatially joined polygone shapefile. I then upload these spatially joined files, and create choloropleth maps based again on the total average number of troops in a given second level administrative region for the years in which I am comparing discussed above:
...
wd=setwd("C:/Users/rsb84/Desktop/RB/COLUMBIA/QMSS/COURSES/Spring_2021/Data Visualization/End_project")
...
acled_2020 = rgdal::readOGR(dsn = paste0(getwd()), layer="acled_2020")
...
CRS.new <- st_crs("EPSG:4269")
active_bases_2020_joined = rgdal::readOGR(dsn = paste0(getwd()), layer="active_bases_2020_joined")
...
map_width <- unit(10, "cm")   # Adjust the width as needed
```



```{r Actual Maps 2016 vs. 2020 - part 3, echo=FALSE, fig.height=6, fig.width=10.5, message=FALSE, warning=FALSE}
...
t <- t + tm_layout(
  inner.margins = c(0.055, 0.155, 0.055, 0.030),   # Increase this value to zoom out more
  outer.margins = c(1.7, 1, 1, 0),          # Increase the first value to add space on the left
  between.margin = 0,
  asp = 1                                    # Set aspect ratio to 1 to keep the maps square-shaped
)
